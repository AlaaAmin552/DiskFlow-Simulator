...
// SCAN algorithm with movement order
int scan(int *requests, int initial_head, int size, int max_cylinders) {
    int total_movement = 0;
    int current_head = initial_head;
    int sorted[size];
    ...
    printf("SCAN Order: %d", current_head);
...
int main() {
    int queue_size, initial_head, max_cylinders, choice;
    ...
    if (choice < 1 || choice > 3) {
        printf("Error: Invalid choice. Please choose between 1 and 3\n");
        free(requests);
        return 1;
    }
    ...
    switch (choice) {
        case 1:
            ...
            break;
        case 2:
            total_movement = scan(requests, initial_head, queue_size, max_cylinders);
